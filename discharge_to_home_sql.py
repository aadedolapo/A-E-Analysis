...
# Load in csv data
csv_file = pd.read_csv('DischargeToHome.csv')
...
try:
    with psy.connect(
        ...
        with conn.cursor() as cursor:
            cursor.execute("DROP TABLE IF EXISTS discharge_to_home")

            script = """CREATE TABLE discharge_to_home (
                            id      int PRIMARY KEY,
                            Place           varchar(10),
            ...
            for index, row in csv_file.iterrows():
                insert_data = """ INSERT INTO discharge_to_home (id,Place,Provider_Name,Age_Group,
